...
<cfinvoke component='script.registration' method='regPeriod' regTerm="#reportTerm#" returnVariable='reportDates'></cfinvoke>
...
<!--- <cfoutput>Ndx = #ndx#<br /></cfoutput> --->
  ...
  <!--- <cfoutput>Day Count = #dayCount#<br /></cfoutput> --->
    ...
    <cfif dayCount lt day(reportDates[x].rptDate)>
		...
		<cfoutput><td align="center"><a href="./index.cfm?action=QL_DRR&sequence=#reportDates[x].sequence#&term=#reportDates[x].term#"><b>#day(reportDates[x].rptDate)#</b></a></td></cfoutput>
   	<cfset counter += 1>
   	<cfif counter is 8>
   		<cfoutput></tr></cfoutput>
