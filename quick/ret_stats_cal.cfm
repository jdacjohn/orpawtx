...
<cfinvoke component='script.registration' method='retStatPeriod' retStatTerm='#regTerm#' returnVariable='reportDates'></cfinvoke>
...
<cfelse> <!--- no reportDates found. --->
	<cfset monthEnds[monthNdx] = ArrayLen(reportDates)>
</cfif>
...
<!--- <cfoutput>Ndx = #ndx#<br /></cfoutput> --->
  ...
  <!--- <cfoutput>Day Count = #dayCount#<br /></cfoutput> --->
    ...
    <cfif dayCount lt day(reportDates[x].rptDate)>
		...
		<cfoutput><td align="center"><a href="./index.cfm?action=QL_RetStats&sequence=#reportDates[x].sequence#&term=#reportDates[x].term#"><b>#day(reportDates[x].rptDate)#</b></a></td></cfoutput>
   	<cfset counter += 1>
   	<cfif counter is 8>
   		<cfoutput></tr></cfoutput>
